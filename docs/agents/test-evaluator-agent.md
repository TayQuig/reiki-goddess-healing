...
describe("ContactForm", () => {
  it("should use SecurityValidator from shared-utils", () => {
    ...
    const result = SecurityValidator.validateContactFormField(
      "email",
      testEmail
    );
...
describe("BookingService", () => {
  describe("Appointment Scheduling", () => {
    it("should prevent double booking same practitioner", async () => {
      ...
      await expect(bookingService.schedule(conflictingBooking)).rejects.toThrow(
    ...
    it("should enforce buffer time between sessions", async () => {
      ...
      await expect(bookingService.schedule(secondSession)).rejects.toThrow(
        "Insufficient buffer time between sessions"
      );
    });
  });
});
...
_This agent is specifically configured for The Reiki Goddess Healing project's testing needs, focusing on wellness industry requirements, security patterns, and maintaining high-quality tests throughout the migration process._

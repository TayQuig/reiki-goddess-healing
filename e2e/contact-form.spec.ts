...
test.describe("Contact Form Security & Functionality", () => {
  ...
  test("should display contact form with proper accessibility", async ({
    page,
  }) => {
  ...
  test("should enforce rate limiting", async ({ page }) => {
    ...
    await expect(
      page.locator("text=maximum number of submissions")
    ).toBeVisible();
...
test.describe("Cookie Consent Banner", () => {
  test("should display cookie consent banner on first visit", async ({
    ...
    await expect(
      page.locator('button:has-text("Accept Necessary Only")')
    ).toBeVisible();
    await expect(page.locator('button:has-text("Accept All")')).toBeVisible();
    await expect(
      page.locator('button:has-text("Customize preferences")')
    ).toBeVisible();
  });
  ...
  test("should save consent preferences when accepting all", async ({
    page,
  }) => {
    await page.evaluate(() => localStorage.clear());
    await page.goto("/");
  ...
  test("should allow customizing preferences", async ({ page }) => {
    ...
    await expect(
      page.locator('h3:has-text("Cookie Preferences")')
    ).toBeVisible();

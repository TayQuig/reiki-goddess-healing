  ...
  test('should be keyboard navigable', async ({ page }) => {
    ...
    for (let i = 0; i < maxTabs; i++) {
      ...
      if (currentElement) {
        ...
        const isInteractive = interactiveElements.includes(currentElement) || 
          await page.evaluate(() => document.activeElement?.hasAttribute('tabindex'));
  ...
  test('should have proper button and link text', async ({ page }) => {
    ...
    for (const button of buttons) {
      ...
      if (text) {
        ...
        const isGeneric = genericTerms.some(term => text.toLowerCase().trim() === term);
  ...
  test('should have proper error message associations', async ({ page }) => {
    ...
    for (const errorMessage of errorMessages) {
      ...
      if (fieldId) {
        ...
        const ariaDescribedby = await associatedField.getAttribute('aria-describedby');

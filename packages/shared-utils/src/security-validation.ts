...
export const VALIDATION_PATTERNS = {
  ...
  forbiddenHealthTerms:
    /\b(diagnosis|prescription|medication|symptom|illness|disease|condition|treatment|medical|doctor|physician)\b/i,
} as const;
...
export class SecurityValidator {
  static validateContactFormField(
    ...
    if (trimmedValue.length > maxLengths[fieldType]) {
      errors.push(
        `${fieldName} exceeds maximum length of ${maxLengths[fieldType]} characters`
      );
      riskLevel = "MEDIUM";
    }
    ...
    if (
      fieldType === "message" &&
      VALIDATION_PATTERNS.forbiddenHealthTerms.test(trimmedValue)
    ) {
      errors.push(
        "Please contact us directly for health-related inquiries to ensure privacy"
      );
      riskLevel = "MEDIUM";
    }

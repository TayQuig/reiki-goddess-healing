...
describe("SecurityValidator", () => {
  describe("validateContactFormField", () => {
    ...
    it("should validate email format", () => {
      ...
      const result2 = SecurityValidator.validateContactFormField(
        "email",
        invalidEmail,
        "email"
      );
    ...
    it("should detect XSS patterns in input", () => {
      ...
      expect(
        result.errors.some((error) =>
          error.includes("potentially malicious content")
        )
      ).toBe(true);
    ...
    it("should detect health information in messages", () => {
      const healthMessage =
        "I need help with my medical condition and prescription medication";
      const result = SecurityValidator.validateContactFormField(
        "message",
        healthMessage,
        "message"
    ...
    it("should sanitize HTML from messages", () => {
      const messageWithHtml =
        'Hello <b>world</b> <script>alert("test")</script>';
      const result = SecurityValidator.validateContactFormField(
        "message",
        messageWithHtml,
        "message"
...
describe("VALIDATION_PATTERNS", () => {
  ...
  it("should detect forbidden health terms", () => {
    expect(
      VALIDATION_PATTERNS.forbiddenHealthTerms.test(
    ...
    expect(
      VALIDATION_PATTERNS.forbiddenHealthTerms.test("My doctor said")
    ).toBe(true);
    expect(
      VALIDATION_PATTERNS.forbiddenHealthTerms.test(
        "I feel stressed and need relaxation"
      )
    ).toBe(false);
    expect(
      VALIDATION_PATTERNS.forbiddenHealthTerms.test(
        "Looking for wellness services"
      )
    ).toBe(false);
  });
});
